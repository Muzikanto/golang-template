...
func NewUser(firstName, lastName, email, password string) (UserDomain, error) {
	user := UserDomain{
		FirstName: firstName,
		LastName:  lastName,
		Email:     email,
...
func (user *UserDomain) Update(firstName, lastName, email string) error {
	if len(firstName) > 0 {
...
func (user *UserDomain) ChangePassword(newPassword string, crypto crypto.Crypto) error {
...
func (user *UserDomain) Validate() error {
...
func (user *UserDomain) ComparePassword(password string, crypto crypto.Crypto) bool {
	return crypto.CompareHashAndPassword(user.Password, password)
}

func (user *UserDomain) HashPassword(crypto crypto.Crypto) error {
